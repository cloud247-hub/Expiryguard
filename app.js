(() => {
  ...
  function showAuthGate(message = 'Logg inn med Microsoft for å åpne ExpiryGuard.', error = false) { document.body.classList.remove('app-authenticated'); els.authGate.hidden = false; els.appMain.hidden = true; els.appFooter.hidden = true; els.authUser.hidden = true; setAuthStatus(message, error); }
  function showApp() { document.body.classList.add('app-authenticated'); els.authGate.hidden = true; els.appMain.hidden = false; els.appFooter.hidden = false; }
  ...
  function renderItems() {
    ...
    els.itemsBody.innerHTML = visible.map(i => {
      ...
      return `<tr data-id="${esc(i.id)}"><td data-label="${L('Kunde', 'Customer')}"><span class="item-title">${esc(tenantName(i.tenantId))}</span></td><td data-label="${L('Navn', 'Name')}"><button class="link-button open-detail" type="button"><span class="item-title">${esc(i.name)}</span><span class="item-sub">${esc(sourceLabel(i.source))}${i.owner ? ` · ${esc(i.owner)}` : ''}</span></button></td><td data-label="${L('Anbefalt start', 'Recommended start')}"><span class="item-title">${fmtDate(recommendedStartAt(i), false)}</span><span class="item-sub">${p.startDays} ${L('dager før', 'days before')}</span></td><td data-label="${L('Utløper', 'Expires')}">${fmtDate(i.expiresAt)}</td><td data-label="${L('Tid igjen', 'Time left')}"><span class="live-expiry countdown" data-item-id="${esc(i.id)}">${formatRemaining(i.expiresAt)}</span></td><td data-label="${L('Plan', 'Plan')}"><span class="badge ${stage}">${stageLabel(stage)}</span><span class="item-sub">${esc(actionText(i))}</span></td><td data-label="${L('Arbeid', 'Work')}"><span class="workflow-badge ${workflowClass(i.workflowState)}">${esc(workflowLabel(i.workflowState))}</span></td><td data-label="${L('Handlinger', 'Actions')}"><div class="row-actions"><button class="icon-button open-detail" type="button" title="${L('Detaljer', 'Details')}">→</button>${i.url ? `<a class="icon-button" href="${esc(i.url)}" target="_blank" rel="noopener noreferrer" title="${L('Åpne', 'Open')}">↗</a>` : ''}${isManual && canWrite() ? `<button class="icon-button edit-manual" type="button" title="${L('Rediger', 'Edit')}">✎</button><button class="icon-button delete-manual" type="button" title="${L('Slett', 'Delete')}">×</button>` : ''}</div></td></tr>`;
    }).join('');
